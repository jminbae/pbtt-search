...
(function (global) {
  ...
  // 메인 검색 함수
  // 다중 단어(공백 split)는 AND 조건. 각 단어별 점수 합산 (모두 매칭되어야 결과에 포함)
  function search(query) {
    ...
    const wordTokenSets = words.map(w => expandQuery(w));
    const results = [];

    for (const qa of ALL_QAS) {
      let allMatch = true;
      let totalScore = 0;
      for (let i = 0; i < wordTokenSets.length; i++) {
        const sc = scoreQA(qa, wordTokenSets[i], words[i]);
  ...
  function highlight(text, query) {
    ...
    // 사용자가 입력한 단어만 mark (SYNONYM 확장 X — 검색 안 친 단어가 강조되는 어색함 방지)
    // 다중 단어(AND)는 공백으로 split해서 각각 mark
    const words = trimmed.split(/\s+/).filter(w => w.length >= 1);
    if (words.length === 0) return escaped;

    // 각 단어를 한 글자씩 분해 + 사이에 \s* 삽입 → "티타늄리프팅"이 본문 "티타늄 리프팅"에도 매칭
