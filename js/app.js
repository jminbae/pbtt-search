...
(function () {
  ...
  // 검색어가 등록된 키워드면 그 카테고리로 자동 전환 (입력 즉시 연동)
  function autoSwitchCategoryIfKeyword(query) {
    const q = (query || '').trim();
    if (!q) return false;
    const norm = q.toLowerCase().replace(/\s+/g, '');
    let cat = KEYWORD_CATEGORY[q];
    if (!cat) {
      for (const k of Object.keys(KEYWORD_CATEGORY)) {
        if (k.toLowerCase().replace(/\s+/g, '') === norm) { cat = KEYWORD_CATEGORY[k]; break; }
      }
    }
    if (cat && cat !== _activeCategory) {
      _activeCategory = cat;
      renderCategoryTabs();
      renderCategoryChips(_activeCategory);
      return true;
  ...
  function cardHtml(qa, query, isSinglePage = false) {
    ...
    const isHot = HOT_IDS && HOT_IDS.has(qa.id);
    const isPick = PICK_IDS && PICK_IDS.has(qa.id);
    let cardCls = 'qa-card';
  ...
  function renderResults(query) {
    ...
    if (headerEl) {
      if (effectiveQuery) {
        ...
        headerEl.innerHTML = `<strong>"${escapeHtml(effectiveQuery)}"</strong>에 대한 ${results.length}개의 답변`;
      } else {
  ...
  function runSearch(query) {
    ...
    renderResults(query);
    // 검색어가 등록 키워드면 카테고리 자동 전환 (전환 안 되면 기존 카테고리 유지하면서 강조만)
    if (!autoSwitchCategoryIfKeyword(query)) highlightChips(query);
  }
  ...
  function handleRoute() {
    ...
    if (qa) {
      ...
      // 메타 복원
      document.title = '피부텐텐 Q&A. 피부과 전문의가 전하는 예뻐지는 모든 이야기';
      renderResults(q);
    }
    highlightChips(q);
  }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (form && input) {
      ...
      input.addEventListener('input', () => {
        ...
        // 입력 즉시 카테고리 자동 전환 (등록 키워드인 경우만 전환됨)
        if (!autoSwitchCategoryIfKeyword(q)) highlightChips(q);
        debouncedSearch(q);
      });
    }
